...
// 30秒快速压力测试 - 验证 k6 连接问题是否已修复
export const options = {
  stages: [
    { duration: '5s', target: 3 },    // 5秒内达到3并发（降低初始负载）
    { duration: '10s', target: 3 },    // 保持3并发10秒
    { duration: '5s', target: 5 },     // 5秒内达到5并发
    { duration: '10s', target: 5 },     // 保持5并发10秒
    { duration: '5s', target: 0 },      // 5秒内降至0
  ],
  thresholds: {
...
function generateJobPayload(type) {
  const payloads = {
    send_email: {
      ...
      body: 'This is a test email from 30s stress testing',
    },
    process_video: {
...
export function setup() {
  ...
  if (!healthCheck) {
    ...
    console.log('✅ Health check passed. Starting 30s stress test...');
  }
}
...
export function teardown() {
  console.log('✅ 30s stress test completed');
}
